...
class SuperNet(nn.Module):
    ...
    def __init__(self, block_args, choices, num_classes=1000, in_chans=3, stem_size=16, num_features=1280,
        ...
        self.conv_stem = create_conv2d(self._in_chs, stem_size, 3, stride=2, padding=pad_type)  # conv stem
        self.bn1 = norm_layer(stem_size, **norm_kwargs)         #
        self.act1 = act_layer(inplace=True)
        self._in_chs = stem_size

        # Middle stages (IR/ER/DS Blocks)   make stage
        ...
        for layer_idx, (layer, layer_arch) in enumerate(zip(self.blocks, architecture)):
            ...
            for block_idx, (blocks, arch) in enumerate(zip(layer, layer_arch)):
                ...
                if backbone_index is not None:
                    ...
                    if layer_idx == backbone_index[0] + 1 and block_idx ==  [1]:
                        return x
            # idx += 1
        return x

    # here, backbone forward (features(architecture, backbone_index))
...
def search_for_layer(flops_op_dict, arch_def, flops_minimum, flops_maximum):
    sta_num = [1, 1, 1, 1, 1]                       # 用前几个？
    order = [2, 3, 4, 1, 0, 2, 3, 4, 1, 0]          # TODO
    limits = [3, 3, 3, 2, 2, 4, 4, 4, 4, 4]         # TODO
    size_factor = 7
    ...
    # print(arch_def)
    #  1 2 4 4 4 4 1, 截取原 arch def 中的每个stage的 1 2 4 4 4 4 1 作为arch def
...
def _gen_supernet(flops_minimum=0, flops_maximum=600, DP=False, **kwargs):
    ...
    model_kwargs = dict(
        block_args=decode_arch_def(arch_def),               # decode arch define
        choices=choices,
